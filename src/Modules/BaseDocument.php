...
class BaseDocument
{
    ...
    */
    ...
    {
        if(!in_array($this->doctype, app('laravel-erp')->local->valid_columns)){
            $valid = $this->meta->get_valid_columns();

            app('laravel-erp')->local->valid_columns[$this->doctype] = $valid;
        }

        return app('laravel-erp')->local->valid_columns[$this->doctype];
    }
     ...
     * @throws \LaravelErp\Exceptions\DoesNotExistError if the table field doesn't exist.
    ...
    */
    protected function get_table_field_doctype($fieldname)
    {
        ...
        }catch (\LaravelErp\Exceptions\DoesNotExistError $e) {
    ...
    public function __get($property)
    {
        if ($property === 'meta') {
            if (!property_exists($this, '_meta')) {
                $this->_meta = app('laravel-erp')->get_meta($this->doctype);
            }
